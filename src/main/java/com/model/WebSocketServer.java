...
public class WebSocketServer {
	...
	public void open(@PathParam("gameId") String gameId, Session session) {
		...
		System.out.println("gameid = " + game.getUniqueGameIdentifier());
		...
		try {
			...
			session.getBasicRemote().sendText(
					MessageService.message(new ClientDataWrapper(gameMap.get(gameId), player, State.CONNECT)), true);
		...
		if (gameMap.get(gameId).getPlayerList().size() == gameMap.get(gameId).getNumOfPlayers()) {
			...
			for (Player p : gameMap.get(gameId).getPlayerList()) {
				...
				System.out.println("...................");
				try {
					p.getSession().getBasicRemote().sendText(
							MessageService.message(new ClientDataWrapper(gameMap.get(gameId), p, State.START)), true);
	...
	public void close(Session session) {
		...
		for (String key : gameMap.keySet()) {
			...
			if (gameMap.get(key).getPlayerList().size() == 0) {
				System.out.println("game with key: " + key + "has been removed");
				gameMap.remove(key);
			}
		}
	...
	public void onMessage(@PathParam("gameId") String gameId, String message, Session session) {
		...
		if (clientMessageConverter.getMessage().equals("CHANGE")) {
		...
		for (Player p : gameMap.get(gameId).getPlayerList()) {
			try {
				p.getSession().getBasicRemote().sendText(
						MessageService.message(new ClientDataWrapper(gameMap.get(gameId), p, State.UPDATE)), true);
